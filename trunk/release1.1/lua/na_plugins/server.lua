...
function ListMaps( ply, params )
	...
	if ply:GetNetworkedBool("ReceivedMaps") ~= true then
		for _, v in pairs(maps) do
			umsg.Start( "NA_AddMap", ply )
				umsg.String( string.Left( v, string.len(v) - 4) )
			umsg.End()
		end
	end
...
if CLIENT then
	function NA_AddMap( um )
		AddMap( um:ReadString() )
	end
	usermessage.Hook( "NA_AddMap", NA_AddMap )
end
...
if SERVER then
	...
	function CreateCountdown( ply, params )
		...
		c_notedstop = false
		for _, v in pairs(player.GetAll()) do
			umsg.Start( "NA_CreateCountdown", v )
				umsg.Long( params[1] )
				umsg.String( c_text )
			umsg.End()
		end
	...
	function NotifyGuys( ply )
		...
		umsg.Start( "NA_CreateCountdown", ply )
	...
	if CLIENT then
		function NA_CreateCountdown( um )
			CreateCountdown( um:ReadLong(), um:ReadString() )
		end
		usermessage.Hook( "NA_CreateCountdown", NA_CreateCountdown )
	end
