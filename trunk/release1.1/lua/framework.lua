...
if CLIENT then
	function NA_Notification( um )
		local Message = um:ReadString()
		local Icon = um:ReadLong()
		
		NA_AddNotify( Message, Icon, 8 )
		surface.PlaySound( "ambient/water/drip" .. math.random(1, 4) .. ".wav" )
	end
	usermessage.Hook( "NA_Notify", NA_Notification )
end
...
if CLIENT then
	function NA_SyncTime( um )
		SyncTimeCL( um:ReadLong() )
	end
	usermessage.Hook( "NA_SyncTime", NA_SyncTime )
end
...
if CLIENT then
	function NA_IsOnServer( um )
		NewAdminOn = true
		NewAdminOnServer = true
	end
	usermessage.Hook( "NA_Available", NA_IsOnServer )
end
