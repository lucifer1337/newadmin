...
if SERVER then
	...
	function NA_UpdateBanlist( ply )
		for i, m in pairs(BannedPlayers) do
			...
			umsg.Start( "NA_AddBan", ply )
				umsg.String( m.Nick )
				umsg.String( m.SteamID )
				umsg.Long( EndTime )
				umsg.String( m.Reason )
				umsg.String( GetNickBySteamID( m.Banner ) )
			umsg.End()
		end
...
if CLIENT then
	function NA_AddBan( um )
		Nick = um:ReadString()
		SteamID = um:ReadString()
		EndTime = um:ReadLong()
		Reason = um:ReadString()
		BannerNick = um:ReadString()
		
		Bans:AddLine( Nick, SteamID, EndTime, Reason, BannerNick )
		table.insert( BanIDs, SteamID )
		
		Bans:SelectFirstItem()
	end
	usermessage.Hook( "NA_AddBan", NA_AddBan )
end
