...
if CLIENT then
	function NA_ShowMenu( um )
		ShowMenu()
	end
	usermessage.Hook( "NA_ShowMenu", NA_ShowMenu )
end

function HideMenu( ply )
	//Listen server hack
	if LocalPlayer == nil then
		umsg.Start( "NA_HideMenu", ply )
			umsg.Bool( true )
		umsg.End()
		return false
	end
...
if CLIENT then
	function NA_HideMenu( um )
		HideMenu()
	end
	usermessage.Hook( "NA_HideMenu", NA_HideMenu )
end
